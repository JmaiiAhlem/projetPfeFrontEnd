...
const Contrats = () => {
  ...
  useEffect(() => {
    const fetchContrat = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/type-contrat`);
        if (response.data.ContratData.length > 0) {
          setContrats(response.data.ContratData);
        } else {
          console.log("No contrats found or empty response.");
        }
      } catch (error) {
  ...
  const deleteContrat = async (ContratId) => {
    try {
      const response = await axios.delete(`http://localhost:3001/type-contrat/${ContratId}`);
    ...
    <div>
      <div className="col-12 bg"><br />
        ...
        <div className="col-12 card"><br />
          <h3 className="card-title col col-xl-3">Liste des Contrats</h3><br />
          <div className="row">
            <div className="empcenter">
              <div className="empcustom-content">
                <div className="emptask-container" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                  <Link to="/home/contrat/add" className="emp-btn btn-9 btn btn-primary" style={{ width: '200px' }}>
                    <FaPlus /> &nbsp;
          ...
          {currentContrats.length > 0 ? (
            <table className="employee-table" style={{ fontSize: "0.9em", tableLayout: "fixed", width: "100%" }}>
              <thead style={{ borderBottom: "2px solid #ccc" }}>
                <tr>
                  <th style={{ borderRight: "1px solid #ccc", width: "13%", fontSize: "13px" }}>Type de contrat</th>
                  <th style={{ borderRight: "1px solid #ccc", width: "13%", fontSize: "13px" }}>Description de Contrat</th>
                  <th style={{ borderRight: "1px solid #ccc", width: "30%", fontSize: "13px" }}>Durée du contrat</th>
              ...
              <tbody>
                {currentContrats.map((e) => (
                  <tr key={e._id}>
                    ...
                    <td style={{ padding: "5px", width: "12%", fontSize: "13px" }}>{e.descriptionContrat}</td>
                    <td style={{ padding: "5px", fontSize: "13px", width: "20%" }}>{e.duree + " mois"}</td>
