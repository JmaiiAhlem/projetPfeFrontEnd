...
const Banks = () => {
  ...
  useEffect(() => {
    const fetchBanque = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/bank`);
        if (response.data.BankData.length > 0) {
          setBanques(response.data.BankData);
        } else {
          console.log("No Banks found or empty response.");
        }
      } catch (error) {
  ...
  const showSuccessToast = () => {
    ...
    toast.fire({
      icon: "success",
      title: "Banque supprimé avec succès",
      width: "600px",
      padding: "10px 20px",
  ...
  const deleteBanque = async (BanqueId) => {
    try {
      const response = await axios.delete(`http://localhost:3001/bank/${BanqueId}`);
      if (response.status === 200) {
        showSuccessToast();
        setBanques(banques.filter((banque) => banque._id !== BanqueId));
      } else {
  ...
  const filteredContrats = banques.filter((bank) =>
  bank.Bname.toLowerCase().includes(searchTerm.toLowerCase()) || 
  bank.Bcode.includes(searchTerm.toLowerCase())
  );
    ...
    <div>
      <div className="col-12 bg"><br />
        ...
        <div className="col-12 card"><br />
          <h3 className="card-title col col-xl-3">Liste des Banques</h3><br />
          <div className="row">
            <div className="empcenter">
              <div className="empcustom-content">
                <div className="emptask-container" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                  <Link to="/home/bank/add" className="emp-btn btn-9 btn btn-primary" style={{ width: '200px' }}>
                    <FaPlus /> &nbsp;
          ...
          {currentContrats.length > 0 ? (
            <table className="employee-table" style={{ fontSize: "0.9em", tableLayout: "fixed", width: "100%" }}>
              <thead style={{ borderBottom: "2px solid #ccc" }}>
                <tr>
                  <th style={{ borderRight: "1px solid #ccc", width: "13%", fontSize: "13px" }}>Code Banque</th>
                  <th style={{ borderRight: "1px solid #ccc", width: "13%", fontSize: "13px" }}>Nom Banque</th>
              ...
              <tbody>
                {currentContrats.map((e) => (
                  <tr key={e._id}>
                    <td style={{ padding: "5px", width: "12%", fontSize: "13px" }}>{e.Bcode}</td>
                    <td style={{ padding: "5px", width: "12%", fontSize: "13px" }}>{e.Bname}</td>
                    <td className="employee-actions" style={{ padding: "5px" }}>
                      <Link to={`/home/bank/edit/${e._id}`} className="customedit btn-sm me-2">
                        <FaEdit />
                      </Link>
                    </td>
                    <td className="employee-actions" style={{ padding: "5px" }}>
                      <button className="customdelete btn-sm" onClick={() => deleteBanque(e._id)}>
                        <FaTrash />
                      </button>
                    </td>
                  </tr>
